...
For using other server than the one defined on index 0 set context value `edgefunctionsinstance_edgefirewall.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `edgefunctionsinstance_edgefirewall.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `edgefunctionsinstance_edgefirewall.ContextOperationServerIndices` and `edgefunctionsinstance_edgefirewall.ContextOperationServerVariables` context maps.
...
auth := context.WithValue(
		context.Background(),
		edgefunctionsinstance_edgefirewall.ContextAPIKeys,
		map[string]edgefunctionsinstance_edgefirewall.APIKey{
